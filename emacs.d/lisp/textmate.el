...
(defcustom tm/exclude-list (list "help-mode" "Info-mode" "term-mode" "shell-mode")
  "List of major-modes where you don't want TM activated."
  :type '(repeat string)
  :group 'textmate)

(dolist (mode tm/exclude-list)
  (add-hook (intern (concat mode "-hook"))
            (lambda nil (setq tm/dont-activate t)) t))
...
(defun tm/pair-insert (arg)
  ...
  (let ((ignore-list (car (last (assoc major-mode tm/non-insert-alist))))
    ...
    (cond
     ...
     ;; acme
     ((and (eq last-command-event ?\") (eq (char-after) ?\"))
      (forward-char))
     ;; end acme
