...
;; (defcustom iimage-mode-image-filename-regex
;;   (concat "[-+./_0-9a-zA-Z]+\\."
;; 	  (regexp-opt (nconc (mapc #'upcase
;; 				     image-file-name-extensions)
;; 			     image-file-name-extensions)
;; 		      t))
;;   "*What the name of an image looks like? That list will tell."
;;   :type 'string
;;   :group 'iimage)

(defcustom iimage-mode-image-extensions-list
  (append (mapc #'upcase
                  image-file-name-extensions)
          image-file-name-extensions)
  "*What the extension of an image looks like? That list will tell."
  :type '(repeat string)
  :group 'iimage)
...
(defcustom iimage-mode-image-filename-regex
  (concat "[-+./_0-9a-zA-Z]+"
          "\\(\."
          (regexp-opt iimage-mode-image-extensions-list t)
          "\\)?")
  "*What the name of an image looks like? That regexp will tell."
  :type 'string
  :group 'iimage)
...
With numeric ARG, display the images if and only if ARG is positive."
  ...
  (let ((ing (if (numberp arg)
	...
	file tempfile buffer-read-only extension)
    (save-excursion
      (goto-char (point-min))
      (dolist (pair iimage-mode-image-regex-alist)
	(while (re-search-forward (car pair) nil t)
	  (if (and (setq tempfile (match-string (cdr pair)))
                   (or (setq file (iimage-locate-file tempfile
                                                      (cons default-directory
                                                            iimage-mode-image-search-path)))
                       (setq file (iimage-locate-file (concat  tempfile ".")
                                                      (cons default-directory
                                                            iimage-mode-image-search-path)
                                                      iimage-mode-image-extensions-list))))
            (if ing
